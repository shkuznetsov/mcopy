...
module.exports = class CopyCollection extends JobsCollectionAsync {
...
class CopyJob extends Job {
	...
	copy () {
		return new Promise((resolve, reject) => {
			...
			let read = fs.createReadStream(this.src.path, {
				highWaterMark: this.collection.opt.highWaterMark
			});
			let write = fs.createWriteStream(this.dest.path);
			// Initialise progress updater
			let progressInterval = setInterval(() => {
				this.collection.emitter.emit('progress', this.src, this.dest, write.bytesWritten);
			}, this.collection.opt.progressUpdateInterval);
	...
	deleteSource () {
		if (this.collection.opt.deleteSource &&
